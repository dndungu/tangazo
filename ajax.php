...
function doCampaignQuery($record, $importID){
	error_log($record[11].' : '.$record[12]);
	error_log(sprintf("(%d, %d, %d, %d, %d, %d, %d, %f, '%s', '%s', %d, UNIX_TIMESTAMP())", $importID, trim($record[14]), trim($record[1]), trim($record[3]), trim($record[5]), trim($record[7]), trim($record[9]), trim($record[10]), date('Y-m-d', strtotime($record[11])), date('Y-m-d', strtotime($record[12])), trim($record[13])));
